...
```{r}
# Split genres by | and get the first genre. Group by that genre and sum the revenue
split_genres <- strsplit(movies$genres, "|", fixed = TRUE)

# Get the first genre
first_genre <- sapply(split_genres, `[`, 1)

# Add the first_genre as a new column to the movies data frame
movies$first_genre <- first_genre
genres <- trimws(genres)
movies$genres <- genres
movies <- na.omit(movies[, c("genres", "revenue")])
movies <- aggregate(movies$revenue, by = list(movies$genres), FUN = sum)
colnames(movies) <- c("genres", "revenue")
movies <- movies[order(movies$revenue, decreasing = TRUE), ]
top_1_revenue_genre <- head(movies, 5)
top_1_revenue_genre <- top_1_revenue_genre[, c("genres", "revenue")]
print(top_1_revenue_genre)
```
